...
draw_mu <- function(
  ...
  df_SIR <- tibble::tibble(
    t  = c(SIR$t, t_end),
    mu = c(theta$beta * SIR$I, 0.6) # add a row manually so that the steps do not stop with vertical line.
    )
  df_PD <- tibble::tibble(
  ...
  ggplot2::ggplot(mapping = ggplot2::aes(x = .data$t, y = .data$mu)) +
    ggplot2::geom_step(data = df_SIR, alpha = 0.5, linetype = 2, size = 0.75) +
    ggplot2::geom_step(data = df_PD , alpha = 0.5, size = 0.75) +
    ggplot2::geom_vline(
      xintercept = SIR$t[SIR$X == "t"], col = "red", alpha = 0.4, linetype = 3
    ) +
    ggplot2::geom_vline(
      xintercept = SIR$t[SIR$X == "j"], col = "blue", alpha = 0.4, linetype = 3
    ) +
